...
def check_keys() -> bool:
    ...
    print(f"  {'[ok]  ' if groq else '[FAIL]'} GROQ_API_KEY          (required, primary provider)")
    print(f"  {'[ok]  ' if openrouter else '[skip]'} OPENROUTER_API_KEY    (optional, fallback free models)")
...
def smoke_test_groq() -> bool:
    print("\nSmoke test: calling Llama 3.3 70B via Groq...")
...
if __name__ == "__main__":
    ...
    if missing:
        ...
        sys.exit(1)
    if not has_groq:
        print("\nSet GROQ_API_KEY in .env or your shell before continuing.")
        print("Get a free key at: https://console.groq.com/keys")
        sys.exit(1)
    if not smoke_test_groq():
